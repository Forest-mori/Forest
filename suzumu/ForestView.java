...

	{
		...
		else
		{
			...
			else
			{
                ...
                int barScore = -aModel.getBar();
                int barScore2 = -aModel.getBar2();
				...
				for(Node aTest : aNode.values())
				{
				   ...
				   // System.out.println("名前["+aTest.getName()+"]の位置座標:"+aTest.getNodeLocation());
					...
					aGraphics.drawRect(aPoint.x+barScore2,aPoint.y+barScore,widthes+2,16);
					aGraphics.drawString(aTest.getName(),aPoint.x+2+barScore2,aPoint.y+13+barScore);
				...
				for (Branch aTest : aBranch)
				{
					...
					aGraphics.drawLine(parent.x+widthes+2+barScore2,parent.y+8+barScore,child.x+barScore2,child.y+8+barScore);
				}
			}
